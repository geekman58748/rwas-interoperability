...
async function main() {
    ...
    if (!txHash || !walletAddress || !assetId) {
        console.error("Usage: npx ts-node scripts/submit_proof.ts <tx_hash> <wallet_address> <asset_id>");
        process.exit(1);
    }
    console.log("═══════════════════════════════════════════════════");
    ...
    const sourceProvider = new ethers_1.ethers.JsonRpcProvider(SEPOLIA_RPC_URL);
    const creditcoinProvider = new ethers_1.ethers.JsonRpcProvider(CREDITCOIN_RPC_URL);
    ...
    console.log(`  ✓ Supported chains: ${supportedChains.map((c) => `chainKey=${c.chainKey}`).join(", ")}`);
    // Check that Sepolia (chainKey 1) is supported
    const sepoliaChain = supportedChains.find((c) => c.chainKey === SEPOLIA_CHAIN_KEY);
    if (!sepoliaChain) {
    ...
    const proofBuilder = new usc_sdk_1.ProofBuilder(PROOF_BUILDER_URL);
    // Wait for the block to be attested on Creditcoin
    await proofBuilder.waitUntilHeightAttested(SEPOLIA_CHAIN_KEY, tx.blockNumber, {
    ...
    const ascContract = new ethers_1.ethers.Contract(ASC_ADDRESS, ASC_ABI, signer);
    const txResponse = await ascContract.submitOwnershipProof(proofData.chainKey, proofData.headerNumber, proofData.txBytes, proofData.merkleProof, proofData.continuityProof, walletAddress, BigInt(assetId));
