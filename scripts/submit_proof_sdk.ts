...
async function main() {
  ...
  const sepoliaChain = chains.find((c: any) => c.chainKey === SEPOLIA_CHAIN_KEY);
  console.log(
    `  ✓ Sepolia (chainKey ${SEPOLIA_CHAIN_KEY}): latest attested = ${sepoliaChain?.latestAttestedHeight}`
  );

  // ── Step 2: Fetch proof ──
  console.log("\n[2/6] Fetching proof from Proof Builder...");
  const proofBuilder = new ProofBuilder(proofBuilderUrl);
  ...
  const merkleProofBytes = ethers.AbiCoder.defaultAbiCoder().encode(
    ["tuple(bytes32 root, tuple(bytes32 hash, bool isLeft)[] siblings)"],
    [proofData.merkleProof]
  );
  const continuityProofBytes = ethers.AbiCoder.defaultAbiCoder().encode(
    ["tuple(bytes32[] blockHashes, bytes32 startHash)"],
    [proofData.continuityProof]
  );
  ...
  const tx = await ascWrite.submitOwnershipProof(
    proofData.chainKey,
    proofData.headerNumber,
    proofData.txBytes,
    merkleProofBytes,
